...
var atlas = require('/lib/thirdParty/Atlas/atlas');

Ti.API.info('atlas=' + atlas);
	...
	if(atlas.Geo.enabled())
	{
		var coordinates = atlas.Geo.getCurrentCoordinates(function (e) {
			if (e.success) {
				...
				loc = geo.reverseGeocoder(e.latitude,e.longitude,reverseGeoCallback);
				Ti.API.info(JSON.stringify(loc));
				Ti.API.info("are we there yet? " + loc.country);
				return e;
			}
			else {
				Ti.API.info(JSON.stringify(e));
